...
const std::unordered_set<std::string_view> void_elements = {"area",  "base", "br",   "col",   "embed",  "hr",    "img",
...
void do_render(const node& n, std::ostream& out) {
    std::visit(overloaded{[](std::monostate) {}, [&](const text_node& t) { escape_text(t.content, out); },
                          [&](const raw_node& r) { out << r.html; },
