...
void document_store::refresh(doc_state& d) {
    ...
    opts.on_ccx_span = [&](size_t b, size_t e) { d.ccx_spans.push_back({b, e}); };
...
std::optional<std::string> uri_to_path(std::string_view uri) {
    ...
    for (size_t i = 0; i < rest.size(); ++i) {
        char c = rest[i];
        if (c == '%' && i + 2 < rest.size() && std::isxdigit(static_cast<unsigned char>(rest[i + 1])) &&
...
std::string path_to_uri(std::string_view path) {
    ...
    for (char c : path) {
        ...
        bool unreserved = (uc >= 'A' && uc <= 'Z') || (uc >= 'a' && uc <= 'z') || (uc >= '0' && uc <= '9') || uc == '-' || uc == '_' ||
                          uc == '.' || uc == '~' || uc == '/' || uc == ':';
        if (unreserved) {
            oss << c;
        } else {
...
size_t offset_from_lsp_position(const weasel::compiler::source_buffer& buf, int line, int character) {
    ...
    if (lidx >= buf.line_starts.size())
        return buf.text.size();
    size_t start = buf.line_starts[lidx];
    size_t line_end = (lidx + 1 < buf.line_starts.size()) ? buf.line_starts[lidx + 1] - 1 : buf.text.size();
